...
export default function AssignCurrentRutinaModal({ usuarios, rutinaId }: { usuarios: any[], rutinaId: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await assignRutina(usuarioId, rutinaId);
      setIsOpen(false);
  ...
  return (
    <>
      ...
      >
        ...
        <span className="hidden sm:inline">Asignar a Alumno</span>
      </button>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 animate-in fade-in">
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95">
            <div className="flex justify-between items-center p-4 border-b border-zinc-800">
              <h2 className="text-xl font-bold text-white">Asignar esta Plantilla</h2>
                ...
                <select name="usuarioId" required className="w-full bg-zinc-950 border border-zinc-800 rounded-lg p-3 text-white focus:outline-none focus:border-yellow-500">
                  ...
                  {usuarios.map(u => (
                    <option key={u.id} value={u.id}>
                      ...
                      {u.rutinaId ? " - [Ya tiene otra rutina]" : ""}
                    </option>
                  ))}
                </select>
              </div>
